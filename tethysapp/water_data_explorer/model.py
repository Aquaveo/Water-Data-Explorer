...
class Groups(Base):
    ...
    hydroserver1 = relationship("Hydroserver_Individual_Cuahsi", back_populates ="group", cascade = "all,delete, delete-orphan" )
    hydroserver2 = relationship("Hydroserver_Individual_Sensor", back_populates ="group", cascade = "all,delete, delete-orphan" )
...
class Hydroserver_Individual_Cuahsi(Base):
    __tablename__ = "Hydroserver_Individual_Cuahsi"
    ...
    countries = Column(JSON)
    group_id = Column(Integer, ForeignKey('Group_Hydroserver_Individuals.id'))
    group = relationship("Groups", back_populates="hydroserver1")  # Tile as given by the admin
    #cascade="all, delete-orphan"
    time_updated = Column(DateTime(timezone=True), onupdate=func.now())
...
class Hydroserver_Individual_Sensor(Base):
    __tablename__ = "Hydroserver_Individual_SensorOfThings"
    ...
    countries = Column(JSON)
    group_id = Column(Integer, ForeignKey('Group_Hydroserver_Individuals.id'))
    group = relationship("Groups", back_populates="hydroserver2")  # Tile as given by the admin
