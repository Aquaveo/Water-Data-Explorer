...
def available_variables(request):
    ...
    for server in hydroservers_groups:
        ...
        hydroserver_variable_code_list = (
            hydroserver_variable_code_list + variables_server["variables_codes"]
        )

        hydroserver_variable_list = (
            hydroserver_variable_list + variables_server["variables"]
        )
        ...
        if url_catalog:
            try:
                ...
                if selected_services:
                    views = water.aux._giveServices(services, selected_services)[
                        "working"
                    ]
                    group_obj["views"] = addMultipleViews(
                        request, hs_list=views, group=title, app_workspace=app_workspace
                    )
                else:
    ...
    for hs in hs_list:
        ...
        try:
            ...
            if sites_parsed_json != '"invalid url"':
                ...
                hs_one = Hydroserver_Individual_Cuahsi(
                    title=hs["title"],
                    url=hs["url"],
...
def catalog_group(request):
    ...
    session = SessionMaker()  # Initiate a session
    hydroservers_group = (
        session.query(Groups).filter(Groups.title == specific_group).first()
    )
...
def delete_group(request):
    ...
    if can_delete_permission:
        ...
        if (
            ...
            for group in groups:
                hydroservers_1_group = (
                    session.query(Groups).filter(Groups.title == group)[0].hydroserver1
                )
                hydroservers_2_group = (
                    session.query(Groups).filter(Groups.title == group)[0].hydroserver2
                )
...
def catalog_filter(request, app_workspace):
    ...
    countries_gdf = gpd.read_file(countries_geojson_file_path)
    selected_countries_plot = countries_gdf[
        countries_gdf["name_long"].isin(countries)
    ].reset_index(drop=True)
    json_selected_country = selected_countries_plot.to_json()

    hs_filtered_region = filter_region(
        countries_geojson_file_path, countries, actual_group=actual_group
    )
    ...
    try:
        df_countries = (
            pd.concat(array_sites_region).drop_duplicates().reset_index(drop=True)
        )
    except Exception:
    ...
    try:
        df_vars = (
            pd.concat(array_sites_variables).drop_duplicates().reset_index(drop=True)
        )
    except Exception:
    ...
    if len(list_countries) > 0:
        ...
        if actual_group is None:
            ...
            hydroservers_selected = (
                ...
                .filter(Groups.title == specific_group)[0]
                .hydroserver
            )
        ...
        for indx in range(0, len(hydroserver_name_list)):
            if "country_metadata" in hydroserver_country_list_check[indx]:
                ...
                df2 = pd.DataFrame(
                    {
                        ...
                        "Country": hydroserver_country_list[indx],
                    }
                )
                if len(hydroserver_country_list[indx]) != len(
                    hydroserver_name_list[indx]
                ):
                ...
                if not df_sites_within_countries.empty:
                    ...
                    for site_fullcode_single in df_sites_within_countries[
                        "SiteName"
                    ].tolist():
                        site_full_name = (
                            site_fullcode_single + hydroservers_selected[indx].title
                        )
                ...
                if len(list(countries_selected["admin"])) > 0:
                    ...
                    list_countries_stations["sites"] = []
                    for column_ in trues_onlys_copy[countries_index]:
                        trues_onlys_copy[column_] = np.where(
                            trues_onlys_copy[column_] is True,
                            trues_onlys_copy.index,
        ...
        for hydroservers in hydroservers_selected:
            ...
            if "whos" in hydroservers.url:
                ...
                for variable_single in variables_list:
                    ...
                    sites_json = json.loads(sites_json_object)["soap:Envelope"][
                        "soap:Body"
                    ]["GetSitesResponse"][
                        "GetSitesResult"
                    ]  # noqa E501
                    ...
                    try:
                        ...
                        if "sitesResponse" in sites_json2:
                            ...
                            if type(sites_object) is list:
                                ...
                                hs_json = {}
                                latitude = sites_object["siteInfo"]["geoLocation"][
                                    "geogLocation"
                                ]["latitude"]
                                longitude = sites_object["siteInfo"]["geoLocation"][
                                    "geogLocation"
                                ]["longitude"]
                                site_name = sites_object["siteInfo"]["siteName"]
                                site_name = site_name.encode("utf-8")
                                network = sites_object["siteInfo"]["siteCode"][
                                    "@network"
                                ]
                ...
                try:
                    df_temp = (
                        pd.concat(hs_list_temp).drop_duplicates().reset_index(drop=True)
                    )
                    # print(df_temp)
